...
export default function MassageCard({
  ...
  const handleSave = async () => {
    ...
    if (!Number(form.duration) || Number(form.duration) <= 0) return toast.error("Durée invalide.")
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      const msg = err?.response?.data?.message || err?.message || "Échec de la suppression."
      toast.error("Erreur", { description: String(msg) })
    } finally {
      setLoading(false)
    }
    ...
    <Card className="container-massagepart">
        ...
        <div className="container-content">
          ...
          <div>
            <CardContent className="container-text">
              {editing ? (
                <>
                  <Input id="name" value={form.name} onChange={onChange} placeholder="Nom du massage" disabled={loading} />
                  <Textarea id="description" value={form.description} onChange={onChange} placeholder="Description" disabled={loading} />
                  <div className="grid grid-cols-2 gap-3">
                    <div className="space-y-1">
                      <label className="text-sm text-muted-foreground">Durée (min)</label>
                      <Input id="duration" type="number" min="1" value={form.duration} onChange={onChange} disabled={loading} />
                    </div>
                    <div className="space-y-1">
                      <label className="text-sm text-muted-foreground">Prix (€)</label>
                      <Input id="price" type="number" min="0" step="0.01" value={form.price} onChange={onChange} disabled={loading} />
                    </div>
                  </div>
                  <div className="space-y-1">
                    <label className="text-sm text-muted-foreground">Position</label>
                    <Input id="position" type="number" min="0" value={form.position} onChange={onChange} disabled={loading} />
                  </div>
                </>
              ) : (
                <>
                  ...
                  <div>
                    <p className="duration">Temps : {`${form.duration} min`}</p>
                    <p className="price">Prix : {`${form.price} €`}</p>
                  </div>
                </>
              )}
            </CardContent>
      ...
      {isAdmin && (
        <CardFooter className="flex items-center justify-end gap-2">
          {editing ? (
            <>
              <Button variant="outline" onClick={cancelEdit} disabled={loading}>Annuler</Button>
